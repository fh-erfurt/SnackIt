...
// TODO: customize functions
    ...
    public static function getAddressById($id)
    {
        ...
        try
        {
            if(!empty($id))
            {
                $sql = 'SELECT country, state, zipcode, city, street, number FROM ' . self::tablename() . ' WHERE addressId = ' . $db->quote($id) . ';';
    ...
    public function delete()
    {
        ...
        try
        {
            $sql = 'DELETE FROM ' . self::tablename() . ' WHERE adressId=' . $this->adressId . ';';
            $db->exec($sql);
            return true;
        }
        catch(\PDOException $e)
